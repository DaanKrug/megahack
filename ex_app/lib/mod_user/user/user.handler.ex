defmodule ExApp.UserHandler do
  ...
  def validateToSave(mapParams) do
    ...
    cond do
      ...
      (category == 'external' and MapUtil.get(UserService.loadById(ownerId),:category) == 'admin_master') 
        -> MessagesUtil.systemMessage(100147)
      ...
      (id == -1 and id2 > 0) -> MessagesUtil.systemMessage(100019,[email])
  ...
  def validateToUpdate(id,user,mapParams) do
    ...
    cond do
      ...
      (category == 'external' and MapUtil.get(UserService.loadById(ownerId),:category) == 'admin_master') 
        -> MessagesUtil.systemMessage(100147)
      (validateOwnerAdmin and UserService.isOwner(id)) -> MessagesUtil.systemMessage(100078)
      (validateOwnerAdmin and SimpleMailService.userIsOwner(id)) -> MessagesUtil.systemMessage(100082)
      (validateOwnerAdmin and ModuleService.userIsOwner(id)) -> MessagesUtil.systemMessage(100090)
  ...
  def validateToDelete(id,user) do
    ...
    cond do
      ...
      (ownerAdmin and UserService.isOwner(id)) -> MessagesUtil.systemMessage(100075)
      (ownerAdmin and SimpleMailService.userIsOwner(id)) -> MessagesUtil.systemMessage(100083)
      (ownerAdmin and ModuleService.userIsOwner(id)) -> MessagesUtil.systemMessage(100091)
  ...
  def update(id,user,mapParams) do
    ...
    email = UserValidator.getEmail(mapParams,MapUtil.get(user,:email))
    password = HashUtil.hashPassword(UserValidator.getPassword(mapParams))
    password = StringUtil.coalesce(password,MapUtil.get(user,:password))
    category = UserValidator.getCategory(mapParams,MapUtil.get(user,:category))
    confirmation_code = UserValidator.getConfirmationCode(mapParams,MapUtil.get(user,:confirmation_code))
    oldActive = (MapUtil.get(user,:active) == 1)
    active = GenericValidator.getBool(mapParams,"active",oldActive)
    permissions = cond do
      (oldActive != active) -> MapUtil.get(user,:permissions)
      true -> UserValidator.getPermissions(mapParams)
    end
    ...
    params = [name,email,password,category,permissions,confirmation_code,active]
    cond do
  ...
  defp mailOnRegistered(email,confirmation_code,user) do
    ...
    cond do
      (!(UserService.mailOnRegistered(email,confirmation_code,deletedAt))) -> MessagesUtil.systemMessage(100006)
