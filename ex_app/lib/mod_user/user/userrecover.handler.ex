defmodule ExApp.UserRecoverHandler do
  ...
  defp recoverUser(email,ownerId) do
    ...
    cond do
      ...
      (!(UserService.recoverUser(email,AuthorizerUtil.getDeletedAt(nil,nil)))) -> MessagesUtil.systemMessage(100011)
  ...
  defp sucessfullyRecovered(user,ownerId) do
    ...
    recovered = AppLogService.create(MapUtil.get(userOwner,:id),
                                     MapUtil.get(userOwner,:name),
                                     MapUtil.get(userOwner,:email),
                                     "Recuperou Senha",
                                     "Pessoa/Usuário",
                                     Poison.encode!(user),"",DateUtil.getNowToSql(0,false,false))
    cond do
