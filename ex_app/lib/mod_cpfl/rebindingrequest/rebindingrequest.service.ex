defmodule ExApp.RebindingrequestService do
  ...
  def hasRecent(clientId,consumerUnitId) do
    ...
    sql = """
	      select id from rebindingrequest where a1_clientid = ? 
  ...
  def loadLast(clientId,consumerUnitId) do
	...
	cond do
      ...
      true -> getRebindingrequest(resultset)
    end
  end
  ...
  def loadById(id) do
    sql = """
	      select id, a1_clientid,
          a2_consumerunitid, ownerId from rebindingrequest where id = ? limit 1
	      """
  ...
  def create(paramValues) do
    sql = """
          insert into rebindingrequest(a1_clientid,
  ...
  def loadAll(page,rows,conditions,deletedAt,_mapParams) do
    ...
    cond do
      (nil == resultset or resultset.num_rows == 0) 
        -> [Rebindingrequest.new(0,0,0,0,0)]
  ...
  def delete(id) do
    DAOService.update("update rebindingrequest set deleted_at = ? where id = ?",
                      [DateUtil.getNowToSql(0,false,false),id])
  end
  
  def unDrop(id) do
    DAOService.update("update rebindingrequest set deleted_at = null where id = ?",[id])
  end
  
  def trullyDrop(id) do
    DAOService.delete("delete from rebindingrequest where id = ?",[id])
  end
  ...
  defp parseResults(resultset,totalRows,arrayMap,row) do
    cond do
      ...
      true -> parseResults(resultset,totalRows,
                           arrayMap ++ [getRebindingrequest(resultset,row,totalRows)],row + 1)
    end
  end
  
  defp getRebindingrequest(resultset,row \\ 0,totalRows \\ nil) do
    total = cond do
      (row == 0) -> totalRows
      true -> nil
    end
    Rebindingrequest.new(NumberUtil.toInteger(ResultSetHandler.getColumnValue(resultset,row,0)),
