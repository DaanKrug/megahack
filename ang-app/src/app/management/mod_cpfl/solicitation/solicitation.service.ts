...
export class SolicitationService extends BaseCrudService {

	getUrl(){
		return this.getBase() + '/solicitations';
	}
	...
	getEmptyObject(conditions) :Object{
		...
		return new Solicitation(0,conditions,
	...
	getAll(page,rows,conditions): Promise<Solicitation[]> {
		return new Promise<Solicitation[]>(r => r(super.getAll(page,rows,this.getEmptyObject(conditions))));
	}

	load(id): Promise<Solicitation> {
		return new Promise<Solicitation>(r => r(super.load(id)));
	} 
