...
export class BaseComponent{
	...
	getSucessCodesForMessages(){
		return [200,201,202,203,204,205,206,207,208,209,210,211,212,213,214,215,216,217,218,219,220];
	}
	...
	truncate(text,maxChars){
		if(null == this.stringServicee){
			return text;
		}
		return this.stringServicee.truncate(text,maxChars);
	}
	
	// Do not override!
	preventDefault(event){
		if(this.emptyObject(event)){
			return false;
		}
		try{event.stopPropagation();}catch(e){}
		try{event.cancelBubble();}catch(e){}
	...
	onDateSelect(event,target){
		...
		var m = date.month;
		var y = date.year;
		var value = this.dateServicee.leftZeros(d,2);
		value += '/' + this.dateServicee.leftZeros(m,2);
		value += '/' + y + ' 00:00:00';
	...
	onDateSelectReport(event,target){
		...
		value += '/' + this.dateServicee.leftZeros(m,2);
		value += '/' + y + ' 00:00:00';
		this.patchValueReport(target,value);
	}
	...
	getExtensionsOfFileType(type){
		...
		return '.png, .jpg, .jpeg, .bmp, .gif, .doc, .docx, .pdf, .xls, xlsx, .ppt, .pptx';
	}
	...
	// Do not override!
	checkObjectOnOptions(objectArray,object){
		if(this.emptyObject(objectArray)){
			objectArray = [];
		}
		if(!(objectArray.includes(object))){
	...
	// Do not override!
	uncheckObjectOnOptions(objectArray,object){
		if(this.emptyObject(objectArray)){
			return [];
		}
		return this.removeFromArray(objectArray,object);
	}
	...
	getLabelForKey(objects,key){
	    ...
	    if(key.indexOf(',') != -1){
	    	return this.getLabelForKeyArray(objects,key.split(','))
	    }
	...
	// Do not override!
	private getLabelForKeyArray(objects,keyArray){
		if(this.emptyArray(keyArray)){
			return '';
		}
		var labels = '';
		var size = keyArray.length;
		for(var i = 0; i < size; i++){
			var label = this.getLabelForKey(objects,keyArray[i]);
	...
	processObjectAndValidationResult(validation: any,statusMessage: boolean): boolean{
		...
		if(validation.objectClass != 'ValidationResult'){
			...
			if(validation.objectClass == 'OperationError'){
				validation.msg = !this.emptyString(validation.msg) 
				               ? validation.msg : 'Erro ao processar solicitação.';
			}
    ...
    addValidationStatusMessage(statusCode,msg){
    	if(null!=this.messageEmitterService){
    		this.messageEmitterService.get('addValidationStatusMessage')
    		                          .emit({code: statusCode, message: msg});
    		return;
    	}
